...
function ImportPage() {
  ...
  async function downloadTemplate() {
    await downloadWorkbook("sociohub-residents-template.xlsx", [
      {
        name: "Residents",
        rows: [
          {
            block: "A",
            flat_number: "101",
            resident_name: "Ravi Patel",
            phone: "9000000001",
            email: "ravi@example.com",
            type: "owner",
            property_number: "P-101",
            ugvcl_number: "UG12345",
            share_certificate_number: "SC-001",
            offline: "no",
          },
          {
            block: "A",
            flat_number: "102",
            resident_name: "Priya Shah",
            phone: "9000000002",
            email: "",
            type: "tenant",
            property_number: "",
  ...
  async function onFile(file: File) {
    ...
    let data: RawRow[];
    try {
      data = await readFirstSheet(file);
    } catch (error) {
    ...
    for (const f of (flatsRes.data ?? []) as any[]) {
      flatKeys.add(
        `${(f.blocks?.name ?? "").toLowerCase()}/${String(f.flat_number).toLowerCase()}`,
      );
    }
    ...
    const norm: NormRow[] = data.map((raw, idx) => {
      ...
      const share_certificate_number = pick(raw, [
        "share_certificate_number",
        "Share Cert",
        "share_cert",
      ]);
      ...
      if (type && !["owner", "tenant", "family"].includes(type))
        errors.push("Type must be owner/tenant/family");
  ...
  async function commit() {
    ...
    const existingBlocks = await supabase
      .from("blocks")
      .select("id,name")
      .eq("society_id", societyId);
    ...
    setBusy(false);
    if (errors.length === 0)
      toast.success(`Imported ${flatCount} houses and ${resCount} residents`);
    else toast.warning(`Imported with ${errors.length} issue${errors.length === 1 ? "" : "s"}`);
  }
  const currentStep: 1 | 2 | 3 = result ? 3 : rows.length > 0 ? 2 : 1;
  ...
  async function downloadErrorReport() {
    ...
    await downloadWorkbook("sociohub-import-errors.xlsx", [{ name: "Errors", rows: rowsOut }]);
  }
            ...
            <div className="rounded-lg bg-muted/50 p-3 text-xs text-muted-foreground flex items-start gap-2">
              ...
              <div className="space-y-1">
                <p>
                  Columns:{" "}
                  <code>
                    block, flat_number, resident_name, phone, email, type, property_number,
                    ugvcl_number, share_certificate_number, offline
                  </code>
                </p>
                <p>
                  Set <code>offline=yes</code> for residents who don't use the app. Online residents
                  sign up &amp; join via invite code.
                </p>
                <p>
                  Validation runs on upload. Only valid rows are imported — the file is never
                  partially applied for corrupted data.
                </p>
              </div>
            </div>
          </CardContent>
            ...
            <CardContent className="p-5 space-y-3">
              <div className="flex items-center justify-between gap-2 flex-wrap">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {summary.invalid > 0 && (
                    <StatusChip tone="danger">{summary.invalid} with errors</StatusChip>
                  )}
                </div>
                    ...
                    {rows.slice(0, 300).map((r) => (
                      <tr
                        ...
                        className={cn(
                          ...
                          r.errors.length && "bg-danger-container/40",
                        )}
                      >
              ...
              {rows.length > 300 && (
                <p className="text-[11px] text-muted-foreground">
                  Showing first 300 rows in preview. All valid rows will be imported.
                </p>
              )}
            </CardContent>
          </Card>
