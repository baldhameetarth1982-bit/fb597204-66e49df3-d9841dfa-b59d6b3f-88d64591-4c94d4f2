...
export const Route = createFileRoute("/_admin/admin/societies/$id")({
  head: () => ({
    meta: [
      { title: "Society — Super Admin · SociyoHub" },
      {
        name: "description",
        content: "Subscription, trial and lifecycle controls for one society.",
      },
    ],
  }),
        ...
        <div className="mt-4 rounded-xl border border-border bg-card p-8 text-center">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {nf
              ...
              : na
                ? "Only platform admins can open this page."
                : "Check your connection and try again."}
          </p>
          {!nf && !na && (
            <Button variant="outline" className="mt-4 min-h-11" onClick={() => q.refetch()}>
              Try again
            </Button>
          )}
        </div>
  ...
  return (
    <div className="container-page space-y-6 py-6 pb-[max(6rem,calc(env(safe-area-inset-bottom)+5rem))] md:py-10">
      ...
      <header className="grid gap-3 border-b border-border pb-5 md:grid-cols-[minmax(0,1fr)_auto] md:items-end">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <h1 className="truncate text-2xl font-semibold tracking-tight md:text-[28px] md:leading-[34px]">
              {s.name}
            </h1>
            <StatusChip tone={st.tone}>{st.label}</StatusChip>
          </div>
          <p className="mt-1 text-sm text-muted-foreground">
            {[s.city, s.state].filter(Boolean).join(", ") || "Location not set"} · joined{" "}
            {fmtDate(s.created_at)}
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Button className="min-h-11" onClick={() => setAction("grant")}>
      ...
      <dl className="grid grid-cols-2 gap-px overflow-hidden rounded-2xl border border-border bg-border lg:grid-cols-4">
        ...
        <Kv
          k={isTrial ? "Trial ends" : "Plan until"}
          v={
            isTrial
              ? s.trial_ends_at
                ? fmtDate(s.trial_ends_at)
                : "—"
              : s.plan_expires_at
        ...
        <Kv
          k="Active units"
          v={`${counts.units}${s.total_units ? ` of ${s.total_units}` : ""}`}
          icon={Home}
        />
          ...
          {admins.length === 0 ? (
            ...
            <ul className="divide-y divide-border overflow-hidden rounded-2xl border border-border bg-card">
              {admins.map((a, i) => (
                <li
                  key={i}
                  className="flex min-h-12 items-center justify-between gap-2 px-4 py-2.5 text-sm"
                >
                  <span className="truncate font-medium">{a.name}</span>
                  <span className="shrink-0 text-xs text-muted-foreground">
                    {a.role === "society_admin" ? "Society admin" : "Block admin"} · since{" "}
                    {fmtDate(a.since)}
                  </span>
                </li>
              ))}
          ...
          {activity.length === 0 ? (
            ...
            <ol className="divide-y divide-border overflow-hidden rounded-2xl border border-border bg-card">
              {activity.map((a, i) => (
                <li
                  key={i}
                  className="flex min-h-11 items-baseline justify-between gap-3 px-4 py-2.5 text-sm"
                >
      ...
      <section className="space-y-2">
        ...
        <div className="divide-y divide-border overflow-hidden rounded-2xl border border-border bg-card">
          <ControlRow
            title="Invite code"
            desc="Issue a new code. The old one stops working immediately."
          >
          ...
          <ControlRow
            ...
            desc={
              suspended
                ? "Members regain access with their existing plan."
                : "Members lose access until restored. No data is deleted."
            }
          >
            {suspended ? (
              <Button variant="outline" className="min-h-11" onClick={() => setAction("restore")}>
          ...
          <ControlRow
            title="Cancel plan"
            desc="Ends paid or trial access now. Data is kept; the society can buy a plan again."
          >
            <Button
              variant="outline"
              className="min-h-11 border-destructive/40 text-destructive hover:text-destructive"
              disabled={s.plan_status === "canceled"}
        ...
        <p className="flex items-start gap-1.5 px-1 text-xs text-muted-foreground">
          <ShieldCheck className="mt-0.5 h-3.5 w-3.5 shrink-0" /> Every action is checked on the
          server and recorded in the audit history. Paid plans bought by societies go through
          Razorpay checkout.
        </p>
      </section>
      ...
      <ActionSheet
        ...
        societyId={s.id}
        societyName={s.name}
        currentPlan={s.plan_id}
        plans={
          (plans.data ?? []) as { id: string; name: string; price_monthly_inr: number | null }[]
        }
        onDone={() => {
          setAction(null);
          refresh();
  ...
  return (
    <Button
      ...
      onClick={async () => {
        if (
          busy ||
          !window.confirm("Generate a new invite code? The old code stops working immediately.")
        )
          return;
        setBusy(true);
        const { error } = await supabase.rpc("regenerate_society_invite_code", {
...
function ActionSheet({
  ...
  plans: { id: string; name: string; price_monthly_inr: number | null }[];
  onDone: () => void;
}) {
  const [planId, setPlanId] = useState(
    currentPlan && currentPlan !== "trial" ? currentPlan : "pro",
  );
  ...
  async function submit() {
    ...
    try {
      if (action === "grant") {
        ...
        toast.success(
          `${planName(planId)} granted for ${months} month${months === "1" ? "" : "s"}`,
        );
      } else {
        const { data, error } =
          action === "trial"
        ...
        if (status !== "success") {
          toast.error(ACTION_MESSAGES[status] ?? "Couldn't save. Try again.");
          return;
        }
        toast.success(
          {
            trial: "Trial extended",
            suspend: "Society suspended",
            restore: "Society restored",
  ...
  const titles: Record<Exclude<Action, null>, [string, string]> = {
    grant: [
      "Grant a plan",
      `Activate or extend a paid plan for ${societyName} without charging them.`,
    ],
    trial: [
      "Extend trial",
      "Adds days from today or from the current trial end, whichever is later.",
    ],
    ...
    cancel: [
      "Cancel plan",
      `Paid features for ${societyName} stop immediately. No data is deleted, and this is recorded in the audit history.`,
    ],
  };
  ...
  return (
    <Sheet open={!!action} onOpenChange={(o) => !o && !busy && onClose()}>
      <SheetContent
        side="bottom"
        className="rounded-t-3xl pb-[max(1rem,env(safe-area-inset-bottom))] sm:mx-auto sm:max-w-lg"
      >
        {action && (
          <>
