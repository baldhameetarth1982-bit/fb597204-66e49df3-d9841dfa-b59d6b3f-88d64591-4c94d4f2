-- Stage 1D correctness: authoritative, non-bypassable transactional creator.
--
-- The prior signature accepted a caller-controlled `_canonical_payload text`
-- and hashed that instead of the actual persisted values, letting a direct
-- authenticated caller forge idempotency equivalence. This migration:
--   1) Creates a new function WITHOUT canonical/hash arguments.
--   2) Derives auth.uid(), normalized values, canonical JSON, and SHA-256
--      hash entirely inside PL/pgSQL.
--   3) Requires creation_request_id.
--   4) Accepts only cash / bank_transfer for NEW records.
--   5) Rejects resident payer until canonical membership check exists.
--   6) Enforces plan parity with normalizePlan() from src/lib/plan-features.ts.
--   7) Revokes and drops the previous signature.

CREATE EXTENSION IF NOT EXISTS pgcrypto WITH SCHEMA extensions;

-- Strip execute from the old signature before dropping it, so any lingering
-- session cannot invoke it during migration.
DO $$
BEGIN
  IF EXISTS (
    SELECT 1
...
BEGIN
  ...
  -- Structural input validation (fail-closed on any bad shape).
  ...
  IF _payer_kind NOT IN ('non_member','anonymous') THEN
    -- 'resident' is explicitly denied here until a canonical resident/society
    -- membership check exists. Do not accept unvalidated resident UUIDs.
    RETURN jsonb_build_object('status','invalid_input');
  END IF;

  -- New Stage 1D records may only be Cash or Bank Transfer.
  IF _payment_method NOT IN ('cash','bank_transfer') THEN
    RETURN jsonb_build_object('status','invalid_input');
  END IF;

  -- Amount: positive, finite, ≤ two decimal places, within canonical range.
  ...
  -- Payer identifiers must match payer_kind exactly.
  ...
  -- Payment date: default to today; never in the future (server clock).
  ...
  -- Reference / description length and empty-to-null normalization.
  ...
  -- Authorization: society admin of THIS society or super admin.
  IF NOT (
  ...
  -- Plan entitlement — parity with normalizePlan() in src/lib/plan-features.ts.
  -- Aliases: pro ∈ {pro, standard, growth}. premium ∈ {premium, business, enterprise}.
  -- Trial with unexpired trial_ends_at (or null trial_ends_at) → premium.
  -- Trial expired / cancelled / canceled / past_due / inactive / expired → basic.
  -- non_member_payments requires pro or premium.
  ...
  IF NOT FOUND THEN
    -- Non-enumerating: same shape as forbidden society.
    RETURN jsonb_build_object('status','not_authorized');
  END IF;
  ...
  -- Category must belong to this society and be active.
  ...
  -- Payer must belong to this society and be active (non_member only).
  IF _payer_kind = 'non_member' THEN
  ...
  -- Canonical JSON is built from the EXACT normalized values that will be
  -- persisted, plus the authenticated creator id. The caller cannot influence
  -- the hash independently of what is stored.
  ...
  -- Idempotency lookup by (society, creator, request_id).
...
COMMENT ON FUNCTION public.create_non_member_income_record(
  ...
  'Stage 1D — authoritative transactional creator for non-member income. Derives auth.uid(), normalizes values, and computes SHA-256 over the canonical JSON of the exact stored fields (caller cannot supply canonical/hash data). Requires creation_request_id. Accepts only cash / bank_transfer for new records. Resident payer is refused until a canonical membership check exists. Enforces plan parity with normalizePlan().';
