...
COMMENT ON FUNCTION public.get_resident_finance_transparency(uuid,date,date,integer) IS 'Server-authoritative, plan-gated resident finance projection from the canonical journal. Transaction amounts are signed net-fund movements so compensating reversals display with the opposite sign. Summary omits transaction detail; detailed/admin projections contain no internal identifiers or vendor contacts.';