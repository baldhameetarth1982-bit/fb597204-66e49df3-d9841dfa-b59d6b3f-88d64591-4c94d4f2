...
- Added `tests/integration/accounting-stage3d-live.test.ts` with eight synthetic-society cases covering balanced posting, private helpers, society-admin scope, tenant isolation, vendors, expense concurrency/replay/reversal, income posting, reports/ageing, legacy exclusion, immutability, audit, and durable backfill replay. The canonical disposable workflow and local runner execute it and fail on skips. Runtime remains unobserved here because Docker/Supabase CLI are unavailable; no protected society data was accessed.
- Completed the existing admin surfaces with cash/bank and journal pagination, explicit retry states, and society-scoped vendor add/edit/deactivate controls on Expenses. Resident finance transparency remains intentionally unavailable because the canonical Stage 3D read RPCs are admin-only; it was not simulated with browser-side data.
