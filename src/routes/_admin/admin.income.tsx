...
function IncomePage() {
  ...
  const header = (
    <PageHeader title="Income & analytics" description="SociyoHub revenue by source and plan." />
  );
  if (error)
    return (
      <div className="container-page py-6 md:py-10">
        {header}
        <ErrorState title="Couldn't load income" onRetry={() => refetch()} />
      </div>
    );
  if (isLoading)
    return (
  ...
  return (
    <div className="container-page space-y-6 py-6 md:py-10">
      ...
      <LeadFigure
        ...
        hint={`${totalSocieties} societies across ${plans.length} plans`}
      />
        ...
        {plans.length === 0 ? (
          ...
          <div className="overflow-hidden rounded-2xl border border-border bg-card">
            <div className="hidden grid-cols-[minmax(0,1fr)_8rem_6rem_10rem] gap-4 border-b border-border px-4 py-2 text-xs font-medium text-muted-foreground sm:grid">
              ...
              <span className="text-right">Societies</span>
              <span>Share</span>
            </div>
            <ul className="divide-y divide-border">
              {plans.map((p) => {
                ...
                return (
                  <li
                    key={p.id}
                    className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-x-4 gap-y-1 px-4 py-3 sm:grid-cols-[minmax(0,1fr)_8rem_6rem_10rem]"
                  >
