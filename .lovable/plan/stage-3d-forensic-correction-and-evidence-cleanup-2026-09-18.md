...
- No generated-type hand edits, client-side authorization substitutes, raw financial errors, fake zeros, or optimistic success.    **Do not advance to Stage 3E.**
- Treat the **actual current migration chain** as authoritative; do not blindly retain duplicate/malformed corrections.
- The resident RPC must explicitly establish the caller's active, society-matching residency/membership **before** revealing visibility or plan information.
- Do not assume `resolve_financial_visibility()` provides authorization unless its implementation proves that.
- Resolve the duplicate `get_resident_finance_transparency` definitions so there is one unambiguous final effective definition.
- Do not change the Stage 3C `93/0/0/0/0` contract or behavior.
- Stage 3D runtime claims must be based on **actual disposable DB execution**, not source-contract tests.
- Keep source-contract tests as defense-in-depth only.
- Do not manipulate expected test counts to make CI green. The count must match real meaningful registered tests.
- Keep Stage 3C and Stage 3D report files/gates completely separate.
- Do not weaken RLS, grants, authentication, tenant isolation, accounting integrity, audit guarantees, or idempotency.
- No generated-type hand edits.
- No `src/lib/utils.ts` changes.
- No protected-society access whatsoever.
- No unrelated metadata/SEO work.
- If infrastructure remains unavailable, preserve `implemented_unverified`/`blocked` honestly.

**One extra hard stop:** if Lovable discovers that the migration chain cannot safely converge without modifying an already-applied historical migration, it must **stop before making that change and report the exact dependency** rather than silently rewriting migration history.