...
function TrustScreen() {
  ...
  return <div className="px-5 py-6 space-y-5 pb-24">
    ...
    {!societyId ? <StateCard title="Society unavailable" message="Your active society could not be determined."/> : report.isLoading ? <div className="text-center py-10" aria-label="Loading financial transparency"><Loader2 className="h-5 w-5 animate-spin mx-auto text-muted-foreground"/></div> : report.error ? <StateCard title={/pro or premium plan/i.test((report.error as Error).message) ? "Transparency is plan locked" : /not allowed/i.test((report.error as Error).message) ? "Transparency is restricted" : "Transparency unavailable"} message={(report.error as Error).message} action={<Button variant="outline" onClick={()=>void report.refetch()}>Retry</Button>}/> : report.data ? <>
      ...
      {report.data.visibility !== "summary" && <Card className="rounded-2xl"><CardContent className="p-4"><p className="text-sm font-semibold mb-3">Recent transactions</p>{report.data.transactions.length === 0 ? <p className="text-sm text-muted-foreground text-center py-4">No posted transactions in this period</p> : <ul className="divide-y divide-border">{report.data.transactions.map((entry,index)=><li key={`${entry.transaction_date}-${entry.source_type}-${index}`} className="py-2.5 flex items-center gap-3"><span className={`h-8 w-8 rounded-xl grid place-items-center ${entry.kind==="income"?"bg-success/10 text-success":"bg-destructive/10 text-destructive"}`}>{entry.kind==="income"?<TrendingUp className="h-4 w-4"/>:<TrendingDown className="h-4 w-4"/>}</span><div className="flex-1 min-w-0"><p className="text-sm font-medium truncate">{entry.description}</p><p className="text-[11px] text-muted-foreground">{new Date(`${entry.transaction_date}T00:00:00`).toLocaleDateString()} · {entry.source_type}</p></div><p className={`text-sm font-semibold tabular-nums ${entry.kind==="income"?"text-success":"text-destructive"}`}>{entry.kind==="income"?"+":"−"}{fmt.format(entry.amount)}</p></li>)}</ul>}</CardContent></Card>}
    </> : null}
  </div>;
}
