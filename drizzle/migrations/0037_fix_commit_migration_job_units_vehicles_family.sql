CREATE OR REPLACE FUNCTION public.commit_migration_job(_job_id uuid, _request_id text, _expected_checksum text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
    ...
    LOOP
      ...
      IF _r.action = 'create' THEN
        ...
        SELECT l.canonical_entity_id INTO _reuse_id
         ...
         LIMIT 1;
        -- Re-imports / retries: reuse an existing house with the same label in
        -- the same structure instead of failing or duplicating it.
        IF _reuse_id IS NULL THEN
          SELECT f.id INTO _reuse_id FROM public.flats f
           WHERE f.society_id = _job.society_id
             AND f.block_id IS NOT DISTINCT FROM _block_id
      ...
      ELSIF EXISTS (SELECT 1 FROM public.offline_residents WHERE id=_resident_id AND society_id=_job.society_id) THEN
        ...
        INSERT INTO public.family_members
          ...
          VALUES (NULL, _resident_id, _job.society_id, _flat_id,
                  ...
                  COALESCE(NULLIF(_r.mapped_json->>'relation',''),NULLIF(_r.mapped_json->>'relationship',''),'other'),
                  NULLIF(_r.mapped_json->>'phone',''),
        ...
        IF EXISTS (SELECT 1 FROM public.offline_residents WHERE id=_resident_id AND society_id=_job.society_id) THEN
          ...
          INSERT INTO public.vehicles
            ...
            VALUES (_job.society_id, NULL, _resident_id, _flat_id, _norm,
                    COALESCE(NULLIF(_r.mapped_json->>'vehicle_type',''),NULLIF(_r.mapped_json->>'type',''),'car'),
                    NULLIF(_r.mapped_json->>'make_model',''),
                    COALESCE(NULLIF(_r.mapped_json->>'colour',''),NULLIF(_r.mapped_json->>'color','')), true)
...
EXCEPTION WHEN OTHERS THEN
  -- SQLSTATE only (never message text) so failures can be diagnosed safely.
  RETURN jsonb_build_object('status','operation_failed','sqlstate', SQLSTATE);
END;
$fn$;