...
- Stage 3D implementation remains **implemented_unverified**: exact replay, source-matched reversals, fail-closed income methods, durable backfill requests, cumulative balances, explicit active/non-moved-out resident authorization, universal audit-history immutability including service-role requests, plan/privacy-gated resident transparency, and signed net-fund movements from canonical journals are implemented. Managed migrations `0011` and `0012` enforce immutable history and restrict direct append access to the canonical server-side role. The strict society-admin helper excludes block administrators from society-wide access. Stage 3C and Stage 3D use separate explicit opt-ins over a neutral disposable-runtime guard; Stage 3D no longer enables Stage 3C internally. The exact eleven-case disposable gate exists, but legacy fresh-reset migration convergence, fresh 11/0/0 execution, and authenticated visual evidence remain outstanding.
- Prompt #41 confirmed the repository has two distinct authorities: managed production changes are recorded under `drizzle/migrations/`, while the CLI fresh-reset verifier replays `supabase/migrations/`. Managed `0011` reapplies the final unconditional audit lock and managed `0012` closes browser append access; neither is disposable-only. The CLI replay track is still behind, so fresh-reset convergence is not claimed. Synthetic row cleanup no longer attempts to delete immutable audit history; audit rows remain until the isolated database is destroyed. The Stage 3D report gate now requires one successful suite, exactly 11 total and passed tests, and zero failed, pending, skipped, or todo tests.
