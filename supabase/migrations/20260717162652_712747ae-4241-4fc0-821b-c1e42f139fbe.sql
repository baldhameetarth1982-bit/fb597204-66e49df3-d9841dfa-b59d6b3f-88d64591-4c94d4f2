-- Stage 2E — corrective additive migration.
-- Fix: family/vehicle resident-link lookup must also match the job's
-- source_type, otherwise colliding external keys across two import
-- sources could link to the wrong resident.
...
BEGIN
  ...
  LOOP
    IF _r.action = 'create' THEN
      -- Defensive guard: validation now rejects structure rows in serial mode,
      -- but keep skip-with-status behavior in case an older job slipped through.
  ...
  LOOP
    ...
    -- Stage 2E fix: also match the job's source_type so keys from a
    -- different import source never link to the wrong resident.
  ...
  LOOP
    _resident_id := NULL; _flat_id := NULL;
    -- Stage 2E fix: source_type-scoped resident link lookup.
    SELECT l.canonical_entity_id INTO _resident_id
      FROM public.migration_entity_links l
     WHERE l.society_id = _job.society_id
       AND l.entity_type = 'resident'
