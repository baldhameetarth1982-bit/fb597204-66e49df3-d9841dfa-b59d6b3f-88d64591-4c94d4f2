    ...
    FROM (
      SELECT
        ...
        abs(CASE
          ...
          ELSE COALESCE(sum(debit - credit) FILTER (WHERE account_type = 'expense'), 0)
        END) AS amount
      FROM scoped_lines
...
COMMENT ON FUNCTION public.get_resident_finance_transparency(uuid,date,date,integer) IS 'Server-authoritative, plan-gated resident finance projection from the canonical journal. Summary omits transaction detail; detailed/admin projections contain no internal identifiers or vendor contacts.';