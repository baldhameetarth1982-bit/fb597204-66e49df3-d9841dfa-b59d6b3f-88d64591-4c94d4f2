...
- Use fixed `search_path`, schema-qualified objects, explicit grants, RLS, canonical `audit_log` columns, and safe external error mapping.    **Add these execution safeguards:**  
Do not stop at an audit or plan. Execute the required implementation changes in this task. Before modifying any existing Stage 3D code, preserve all already-correct Stage 3C behavior and existing canonical security controls. For payment/income posting, if any existing Stage 3C invariant conflicts with the new accounting implementation, preserve the Stage 3C invariant and adapt the accounting layer instead.
  Any migration/function/policy that could affect an existing financial path must be checked for fresh-schema and upgraded-schema ordering before being considered complete. Never restore previously revoked direct client financial writes.
  For every new accounting RPC, test both an authorized synthetic-society path and a deliberately unauthorized/cross-society path. Fail closed on authorization, tenant, plan, visibility, source-state, replay, and audit failures.
  Do not mark Stage 3D beyond `implemented_unverified` merely because static tests/typecheck/build pass. Runtime and visual status must reflect actual evidence.