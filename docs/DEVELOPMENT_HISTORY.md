...
- Migration authority was clarified: `supabase/migrations/` is the fresh-reset/production schema track; `drizzle/migrations/` is historical evidence from earlier tooling and must not be replayed alongside it. Drizzle `0009` remains immutable history and is superseded by terminal `0010`; its service-role audit cleanup bypass must never be restored. The production track still requires an additive mirror of the terminal audit lock before disposable convergence can be claimed.
