...
- Handler module rewritten: `tests/helpers/stage3c-live-idempotency-reference-cases.ts` — no `vitest` import, no `!` non-null assertions; every mutation and denial bracketed by `snapshotResidentBillState` / `assertResidentBillStateUnchanged` on the strict `ResidentBillStateSnapshot`. `satisfies Record<Stage3CIdempotencyReferenceCaseId, Stage3CIdempotencyReferenceHandler>`.
- Canonical amounts: `IDEMPOTENCY_AMOUNT = 250`, `IDEMPOTENCY_CONFLICT_AMOUNT = 251`, `REFERENCE_AMOUNT = 200`.
- Bill routing: IDEMPOTENCY-01..04 target `fixture.idempotencyBillId` (1000, dedicated). REFERENCE-01/02 target `fixture.referencePrimaryBillId` (800). REFERENCE-03 targets `fixture.referenceSecondarySameSocietyBillId` (700). REFERENCE-04 targets `fixture.referenceOtherSocietyBillId` (600) via `unrelatedResident`.
- Shared resident production Bank Transfer core is the **exactly one** RPC construction for every REFERENCE path (`fixture.helpers.submitResidentBankTransferPayment` → `submitResidentBankTransferWithClient`). Admin helpers (`submitAdminBankTransferPayment`, `recordAdminOfflinePayment`, `adminA1`, `adminB`) are absent from this slice.
- IDEMPOTENCY semantics enforced: IDEMPOTENCY-01 submits at 250 and identical replay returns the ORIGINAL payment id (row delta = +1). IDEMPOTENCY-02 same key + amount 251 → `idempotency_conflict`, zero delta, state unchanged. IDEMPOTENCY-03 is proof-only: state still identical to the post-primary snapshot and exactly one row for the original payment id survives. IDEMPOTENCY-04 same key + same bill + amount 251 → `idempotency_conflict`, zero delta.
- REFERENCE semantics enforced (society-scoped `upper(trim(reference_no))` uniqueness for bank_transfer): unique reference succeeds on the primary bill; whitespace/case-variant duplicate on the SAME bill and on a DIFFERENT bill in the SAME society both raise `duplicate_reference` with the primary bill state unchanged; the SAME normalized reference in Society B via `unrelatedResident` succeeds (cross-society isolation).
- Matrix context (`tests/helpers/stage3c-live-matrix-context.ts`) now types every lifecycle snapshot as `ResidentBillStateSnapshot | null` (no `unknown` state bags) and adds `idempotencyBillId`, `idempotencyAmountInput`, `idempotencyConflictAmountInput`, `referencePrimaryBillId` slots with matching `require*` guards (`requireIdempotencyBillId`, `requireIdempotencyInitialState`, `requireReferencePrimaryBillId`, `requireReferencePrimaryInitialState`).
- Fixture (`tests/helpers/stage3c-runtime-fixtures.ts`) exposes `idempotencyBillId` (1000) and `referencePrimaryBillId` (800) aliases in addition to `referenceSecondarySameSocietyBillId` (700) and `referenceOtherSocietyBillId` (600).
- Denial control flow: RPC invoked inside a try/catch that only records `{ caught, succeededData }`; the "unexpected success" assertion is thrown OUTSIDE the catch so it can never be consumed by `assertCanonicalError`.
- Payment-id tracking is duplicate-safe (`trackUniqueId`); denials never add ids to `tracked.paymentIds`.
- Source validator `scripts/verify-stage3c-live-matrix-40-source.ts` strengthened to reject wrong amounts, missing dedicated bills, `unknown` snapshot slots, `vitest` imports, non-null assertions, and any admin bank-transfer / admin actor usage in this slice.
- 32-case validator `scripts/verify-stage3c-live-matrix-32-source.ts` retains its workflow boundary (must NOT claim 32/93 or 40/93). Runtime workflow (`.github/workflows/stage3c-runtime-verification.yml`) restored to its pre-slice state and continues to run only the 24-case core report.
- Manifest descriptions for IDEMPOTENCY-03 (proof-only) and IDEMPOTENCY-04 (same-bill amount conflict) updated to match the repaired semantics.
- Focused unit test rewritten: `tests/unit/billing-stage3c-live-idempotency-reference.test.ts`.
...
- Prompt #34 evidence: focused accounting/privacy/authorization tests 49/49; full non-live suite 2,542 passed, 123 skipped, 11 todo; TypeScript and Stage 3C canonical/source validators passed. Build and bundle-secret scan were not rerun because the required build output is unavailable in this sandbox.
- Resident transparency now uses a server-authoritative, plan-gated canonical-journal RPC with `admin/summary/detailed/none` enforcement; summary omits transaction detail and detailed output omits identifiers, references, and vendor contacts. Resident direct legacy-ledger reads were removed. Admin report failures/loading no longer look like genuine zero values.
- Remaining before Stage 3D closure: execute the checked-in eleven-case synthetic posting/reversal/backfill/privacy suite on a fresh disposable database and complete authenticated mobile/desktop accessibility and visual QA. Every Stage 3D entry point now fails closed unless that suite reports exactly 11 passed, 0 failed, and 0 skipped. Therefore Stage 3D remains `implemented_unverified`, not complete. Stage 3E remains `not_started`.
