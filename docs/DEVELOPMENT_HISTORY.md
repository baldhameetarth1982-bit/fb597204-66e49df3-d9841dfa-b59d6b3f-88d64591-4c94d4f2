...
- Added `tests/integration/accounting-stage3d-live.test.ts` with nine synthetic-society cases covering balanced posting, private helpers, society-admin scope, tenant isolation, vendors, expense concurrency/replay/reversal, income posting, reports/ageing, legacy exclusion, immutability, audit, durable backfill replay, and resident summary/detailed/privacy isolation. The canonical disposable workflow executes it and fails on skips. Runtime remains unobserved here because Docker/Supabase CLI are unavailable; no protected society data was accessed.
